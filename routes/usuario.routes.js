...
router.get("/registro", (req, res) => {
  if (req.user) {
    req.flash(
      "error_msg",
      "Para criar uma nova conta, você não pode estar logado"
    );
    res.redirect("/");
  } else {
    ...
    Usuario.findOne({ email: email })
      ...
      .then((usuario) => {
        if (usuario) {
          ...
          req.flash(
            "error_msg",
            "An account with this email already exists in out system"
          );
          res.redirect("/usuarios/registro");
        } else {
          // não existe um usuario com o email cadastrado

          if (email == "viniciusvieira130705@gmail.com") {
              ...
              bcrypt.hash(novoUsuario.senha, salt, (erro, hash) => {
                ...
                novoUsuario
                  ...
                  .catch((err) => {
                    res.flash(
                      "error_msg",
                      "There was an internal error to create the user, try again"
                    );
                    res.redirect("/usuarios/registro");
                  });
              });
